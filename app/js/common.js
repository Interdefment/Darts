	...
	methods: {
		...
		deleteActive: function deleteActive (player, index) {
			if (this.deleteClicker == 2) {
				if (player.id == this.pickedPlayers[this.currentPlayer].id) {
					this.showMessage('Нельзя удалить активного игрока');
					this.deleteClicker = 1;
					return false;
				};
				for (let i = 0; i < this.pickedPlayers.length; i++) {
					console.log(this.currentPlayer);
					if (player.id == this.pickedPlayers[i].id && this.currentPlayer > i) {
						this.currentPlayer--;
					}
				};
				this.pickedPlayers.splice(index, 1);
